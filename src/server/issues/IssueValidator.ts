...
export class IssueValidator implements IIssueValidator {
  ...
  public validateCreate(input: unknown): CreateIssueInput {
    const parsed = this.createSchema.safeParse(input);
    if (!parsed.success) {
      throw new ValidationError("Invalid issue data", parsed.error.flatten());
    }
    return parsed.data;
  }
  ...
  public validateUpdate(input: unknown): UpdateIssueInput {
    const parsed = this.updateSchema.safeParse(input);
    if (!parsed.success) {
      throw new ValidationError("Invalid issue update", parsed.error.flatten());
    }
    return parsed.data;
  }
  ...
  public validateId(id: unknown): string {
    ...
    if (!parsed.success) throw new ValidationError("Invalid issue id");

    // MongoDB ObjectId format: 24 hex characters
    if (!/^[a-fA-F0-9]{24}$/.test(parsed.data)) {
